...
**Where this landed.** The self case works end to end, and the mutual case is contified for **every** group. It was for a long time contified only for a group whose members share a signature — one parameter list and one `ret` have to serve them all — and the three groups in the compiler's own source that do not were reported and emitted as ordinary functions. That was correct C and was **not** §7.7: the only place a documented spec guarantee did not hold in the emitted compiler. A group that does not share a signature now takes a second path which shares neither half: each member gets parameter variables of its own (`q<state>_<k>`, so no two can collide) and the result leaves through an out-parameter, since a C function has one return type and the members have several. The group function's parameters are every member's concatenated, and a wrapper passes a zero compound literal for the members it is not — dead at that state, and C needs something there. One dispatch loop either way, so §7.7 holds either way. The fast path is kept because it is what the emitted compiler's own hot mutual recursions go through.

The indirect case is **not** implemented. The trampoline needs every function to return "a value or a pending call", which is a whole-program calling convention this backend does not have, so an indirect tail call is emitted as a plain call carrying a marker that says so.
...
- **A module's globals are initialised after the importing file's, which is a live miscompile.** §4.10 initialises a `$decl` once in source order and `mpl_init` does it in index order — but the entry file's own top-level items are lifted *before* the members of the modules it imports. So a top-level `$decl` in the entry file that calls into an import reads a zeroed global and segfaults, on a one-line input. Every driver written the obvious way has that shape and has never been compiled; `mplc.mpl` is immune only because everything it does sits inside `main`, which runs after `mpl_init`. Stage 0 does not have the bug, so "it runs under the interpreter" proves nothing here. The fix is to number an imported module's members *before* the importing file's own, which is what §4.14's "`$import` loads a block" means for initialisation order.
- **There is no way to write to stderr, so a compiled compiler can only report a diagnostic by dying.** §8 gives `print` (stdout) and `panic` (stderr, then abort), and nothing in between. `mplc` therefore aborts on a `check` or `verify` failure — correct, and it exits 134 because §7.8 makes `panic` abort — but it cannot *warn*: a non-fatal emit diagnostic stage 0 prints is invisible under it, because printing it would corrupt the C on stdout. `stage1/emit_diag.mpl` is the workaround — the same pipeline with the diagnostics on stdout and the C discarded — and it is what made §6a's three refusals findable at all, turning a ninety-minute stage-0 question into a one-second one. This is the same root as `link` being unreachable: §9 makes `$extern` the only door to the platform and stage 1 has none. §8 wants either a stderr intrinsic or an explicit statement that diagnostics are the build program's problem (§9).
