...
namespace AST {
  ...
  struct ProgramNode : public ASTNode {
    ...
    ProgramNode(std::vector<std::unique_ptr<ASTNode>>&& v) : ASTNode(PROGRAMNODE)
  ...
  struct BlockNode : public ASTNode {
    ...
    BlockNode(std::vector<std::unique_ptr<ASTNode>>&& v) : ASTNode(BLOCKNODE)
                                                         , children_(std::move(v)) {}
  };
  ...
  struct StatementNode : public ASTNode {
    ...
    StatementNode(std::unique_ptr<ASTNode>&& child, bool isPrint) : ASTNode(STATEMENTNODE), 
      child_(std::move(child)), isPrint_{isPrint} {}
  };

  struct IfNode : public ASTNode {
    std::unique_ptr<ASTNode> ifBlock_;

    IfNode(std::unique_ptr<ASTNode>&& ifBlock) : ASTNode(IFNODE), ifBlock_(std::move(ifBlock)) {}
  };

  struct WhileNode : public ASTNode {
    std::unique_ptr<ASTNode> whileBlock_;

    WhileNode(std::unique_ptr<ASTNode>&& whileBlock) : ASTNode(IFNODE), whileBlock_(std::move(whileBlock)) {}
  };
  ...
  struct BinaryOpNode : public ASTNode {
    ...
    BinaryOpNode(TokenType type, std::unique_ptr<ASTNode>&& op1, std::unique_ptr<ASTNode>&& op2)
      : ASTNode(BINARYOPNODE), operand1_(std::move(op1)), operand2_(std::move(op2)), opType_{type} {}
  };
  ...
  struct IdentifierNode : public ASTNode {
    ...
    IdentifierNode(std::string name)
      : ASTNode(IDENTIFIERNODE), name_{name} {}
  };
