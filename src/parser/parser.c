...
static bool append_ast_tokens(TokenBuffer* buf,
  ...
  if (node->kind == AST_BUILTIN && node->op) {
    Str op_name = interns_lookup(interns, node->op);
    if ((op_name.len == 5 && strncmp(op_name.ptr, "$$end", 5) == 0) ||
        (op_name.len == 7 && strncmp(op_name.ptr, "$$delim", 7) == 0)) {
      for (size_t i = 0; i < node->child_count; ++i) {
      ...
      return append_token(buf, end_token);
    }
  }
...
static bool append_group_child(AstNode* group, AstNode* node, InternTable* interns) {
  if (!group || !node) return false;
  if (node->kind == AST_GROUP) {
    for (size_t i = 0; i < node->child_count; ++i) {
      if (!append_group_child(group, node->children[i], interns)) return false;
    }
    return true;
  }
  if (node->kind == AST_BUILTIN && node->op && interns) {
    Str op_name = interns_lookup(interns, node->op);
    if ((op_name.len == 5 && strncmp(op_name.ptr, "$$end", 5) == 0) ||
        (op_name.len == 7 && strncmp(op_name.ptr, "$$delim", 7) == 0)) {
      for (size_t i = 0; i < node->child_count; ++i) {
  ...
  return ast_append_child(group, node);
}
...
static AstNode* build_template_ast(const Production* prod,
  ...
  if ((op_text.len == 7 && strncmp(op_text.ptr, "$spread", 7) == 0) ||
      (op_text.len == 8 && strncmp(op_text.ptr, "$$spread", 8) == 0)) {
    ...
    AstNode* group = ast_new(AST_GROUP);
    if (!group) return NULL;
    for (size_t i = 0; i < cap->count; ++i) {
      if (!append_group_child(group, cap->nodes[i], interns)) {
        ast_free(group);
        return NULL;
      }
    }
  ...
  if (op_cap) {
    ...
    while (probe_rem > 0) {
      ...
      if (probe_len == 7 && strncmp(probe_tok, "$$delim", 7) == 0) {
        saw_end = true;
        continue;
      }
    ...
    if (only_directives) {
      ...
      Sym end_sym = interns_intern(interns, str_from("$$end", 5));
      if (!end_sym) return NULL;
      AstNode* wrapper = ast_new(AST_BUILTIN);
      if (!wrapper) return NULL;
      wrapper->op = end_sym;
      if (op_cap->count == 1) {
        if (!ast_append_child(wrapper, op_cap->nodes[0])) {
          ast_free(wrapper);
          return NULL;
        }
        return wrapper;
      }
      AstNode* group = ast_group_from_list(op_cap->nodes, op_cap->count);
      if (!group || !ast_append_child(wrapper, group)) {
        ast_free(group);
        ast_free(wrapper);
        return NULL;
      }
