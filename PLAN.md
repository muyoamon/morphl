...
Introduce a two-stage VM pipeline with linkable object files (`.mplo`) and runnable executables (`.mple`).
...
- `.mplo` for object files and `.mple` for executables
...
- Add two VM artifact kinds:
  ...
  - `.mple`: runnable linked executable containing finalized code/data layout
- Extend the CLI with explicit VM compile and link modes:
  - `morphlc -c` compiles source to `.mplo` only
  - `mpll` links one or more `.mplo` files to `.mple`
  - `mplvm` runs `.mple`
...
- Restrict the VM runtime to loading `.mple` only; `.mplo` is explicitly non-runnable.
...
- Add a static VM linker that consumes one or more `.mplo` files and produces one `.mple`.
...
- Keep the VM runtime simple: it loads finalized `.mple` only.
...
- Add convenience executables:
  - `mpll` for linking `.mplo` to `.mple`
  - `mplvm` for running `.mple`
...
- Make output naming reflect artifact type defaults:
  ...
  - linked executable output defaults to `.mple`
...
- VM object/link tests:
  ...
  - link one `.mplo` into `.mple` and execute successfully
...
1. Split VM artifacts into non-runnable `.mplo` object files and runnable `.mple` executables, including header-level kind/version validation and runtime rejection of `.mplo` inputs.
